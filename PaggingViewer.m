...
- (UIView *)centerContainerView {
    if (!_centerContainerView) {
        ...
        _centerContainerView.backgroundColor = [UIColor blackColor];
...
- (void)callBackChangedPage {
    if (self.didChangedPageCompleted) {
        self.didChangedPageCompleted(self.currentPage, [[self.viewControllers valueForKey:@"Timeline"] objectAtIndex:self.currentPage]);
    }
}
